...
RSpec.describe 'garden show' do
  ...
  it 'plants sorted by number of appearances in garden' do
    turing_garden = Garden.create!(name: 'Turing Community Garden', organic: true)
    library_garden = Garden.create!(name: 'Public Library Garden', organic: true)
    ...
    tomato = Plant.create!(name: "Tomato", description: "red", days_to_harvest: 30)
    squash = Plant.create!(name: "Squash", description: "yellow", days_to_harvest: 90)
    ...
    within "#plants" do
      ...
      expect(page).to have_content("Pepper", count: 1)
      expect(page).to_not have_content("Lettuce", count: 1) # too slow
      expect(page).to have_content("Cucumber", count: 1)
      expect(page).to_not have_content("Tomato")
      expect(page).to_not have_content("Melon")

      # no orderly
      # expect("Squash").to appear_before("Pepper")
      # expect("Pepper").to appear_before("Cucumber")
    end
  end
end