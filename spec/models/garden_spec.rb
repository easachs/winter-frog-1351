...
RSpec.describe Garden do
  ...
  describe 'instance methods' do
    it 'has distinct fast plants' do
      ...
      expect(turing_garden.fast_plants).to eq([tomato, cucumber, pepper])
      expect(library_garden.fast_plants).to eq([])
    end

    xit 'plants sorted by number of appearances in garden' do
      turing_garden = Garden.create!(name: 'Turing Community Garden', organic: true)
      library_garden = Garden.create!(name: 'Public Library Garden', organic: true)
      ...
      PlantPlot.create(plant: lettuce, plot: plot_1)
      PlantPlot.create(plant: cucumber, plot: plot_1)
      PlantPlot.create(plant: squash, plot: plot_2)
      PlantPlot.create(plant: lettuce, plot: plot_2)
      PlantPlot.create(plant: squash, plot: plot_2)
      PlantPlot.create(plant: squash, plot: plot_2)
      PlantPlot.create(plant: squash, plot: plot_2)
      PlantPlot.create(plant: tomato, plot: plot_3)

      expect(turing_garden.fast_sorted).to eq([squash, lettuce, cucumber])
      expect(library_garden.fast_sorted).to eq([tomato])
    end
  end
end
